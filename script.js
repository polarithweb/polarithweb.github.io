...
async function initializeApp() {
  try {
    // 1. Fetch the data you uploaded to data.json
    ...
    // 3. Teach the AI the newly fetched pricing
    initializeAI(data.pricing);

  } catch (error) {
    console.error("Failed to load external data:", error);
    // Fallback AI prompt if data.json is missing
    initializeAI([]); 
  }
}
...
function initializeAI(pricingData) {
  let pricingString = "No pricing data available at the moment.";
