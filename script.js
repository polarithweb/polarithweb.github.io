...
function parseIssueBody(body) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (header.includes('tag')) {
      result.tag = (content !== 'No Tag') ? content : null;
    }
  }
  return result;
}
...
function renderPortfolio(portfolioData) {
  ...
  portfolioData.forEach(item => {
    ...
    const linkEl = clone.querySelector('.port-link');
    if (item.link) {
...
function initializeAI(pricingData) {
  let pricingString = "No specific pricing data available right now.";
  if (pricingData && pricingData.length > 0) {
    pricingString = pricingData.map(p => `- ${p.title}: ₹${p.price} (${p.features.join(', ')})`).join('\n');
  }

  SYSTEM_PROMPT = `You are Polarith AI, built exclusively on the PriAxom HyperEngine by Priyam Kesh.
TONE: Confident, sharp, highly intelligent. 
CRITICAL RULE 1: YOU MUST ANSWER IN 1 OR 2 SHORT SENTENCES. MAXIMUM 30 WORDS. NEVER write a long paragraph. 
CRITICAL RULE 2: Get straight to the point. Be extremely brief.

PRICING DATA:
${pricingString}

Goal: Recommend the Premium Production Grade package if they ask for the best. Tell users to email priaxom.ai@outlook.com to buy. KEEP IT VERY SHORT.`;

  chatHistory = [
    { role: "system", content: SYSTEM_PROMPT },
    { role: "assistant", content: "Hello! I am Polarith AI, running on the PriAxom HyperEngine. Let's keep it sharp—how can I help you today?" }
  ];
}
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.5,
        max_tokens: 80 // Hard limit to ensure brevity
      })
    });
    ...
    appendMessage('ai', "My connection dropped. Check your network.");
  } finally {
